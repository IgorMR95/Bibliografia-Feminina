...
import { ArrowLeft, Edit, Save, Plus, BookOpen, Trash2, CheckCircle } from "lucide-react";
...
export const DetalheAssociada = () => {
  ...
  const loadData = async () => {
    try {
      const [assocRes, camposRes, prodRes] = await Promise.all([
        supabase
          .from("associadas")
          .select("*, vinculos_docentes(*), notas(*, perfis(nome))")
          .eq("id", id!)
          .single(),
        supabase.from("definicoes_campos_extras").select("*").eq("entidade", "Associada").order("label"),
        supabase.from("producoes_bibliograficas").select("*").eq("associada_id", id!).order("ano_publicacao", { ascending: false }),
      ]);

      if (assocRes.error) throw assocRes.error;

      const initialData = { ...assocRes.data };
      // Spread dados_extras JSONB into top-level fields for the form
      if (initialData.dados_extras && typeof initialData.dados_extras === "object") {
        Object.assign(initialData, initialData.dados_extras);
      }
  ...
  const onSave = async (formData: any) => {
    try {
      ...
      // Extract dados_extras from top-level fields
      const dadosExtras: any = {};
      camposExtras.forEach(c => {
        if (dbPayload[c.nome_campo] !== undefined) {
          dadosExtras[c.nome_campo] = dbPayload[c.nome_campo];
          delete dbPayload[c.nome_campo];
        }
      });
      if (Object.keys(dadosExtras).length > 0) dbPayload.dados_extras = dadosExtras;

      // Remove nested relations from payload
      delete dbPayload.vinculos_docentes;
      delete dbPayload.notas;

      const { error } = await supabase.from("associadas").update(dbPayload).eq("id", id!);
      if (error) throw error;

      // Replace vinculos: delete all + reinsert
      await supabase.from("vinculos_docentes").delete().eq("associada_id", id!);
      if (vinculos.length > 0) {
        await supabase.from("vinculos_docentes").insert(
          vinculos.map((v: any) => ({ tipo: v.tipo, instituicao: v.instituicao, integra_ranking_40: v.integra_ranking_40, associada_id: id! }))
        );
      }
  ...
  const handleAddNota = async () => {
    ...
    try {
      const { error } = await supabase.from("notas").insert({ associada_id: id!, texto: novaNota });
      if (error) throw error;
      setNovaNota("");
      loadData();
    } catch {
  ...
  const handleDeleteProducao = async (prodId: string) => {
    ...
    try {
      const { error } = await supabase.from("producoes_bibliograficas").delete().eq("id", prodId);
      if (error) throw error;
      loadData();
    } catch {
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex space-x-3">
          {user && (!isEditing ? (
            ...
            <>
              <button onClick={() => { setIsEditing(false); reset(data); }} className="px-4 py-2 bg-white border border-[var(--border)] text-[var(--text-main)] text-sm font-semibold rounded-lg hover:bg-[var(--row-hover)] transition shadow-sm">Cancelar</button>
                  ...
                  <div className="flex justify-between items-start">
                    ...
                    {user && (
                      <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition">
                        <button type="button" onClick={() => { setEditingProducao(p); setShowProducaoModal(true); }} className="p-1 hover:text-[var(--accent)]"><Edit className="w-3.5 h-3.5" /></button>
                        <button type="button" onClick={() => handleDeleteProducao(p.id)} className="p-1 hover:text-[var(--error)]"><Trash2 className="w-3.5 h-3.5" /></button>
                      </div>
                    )}
                  </div>
                  {p.link_acesso && (
              ...
              {data.notas?.map((n: any) => (
                <div key={n.id} className="bg-[var(--bg)] p-3 rounded-lg border border-[var(--border)] text-sm">
                  <div className="flex justify-between items-start mb-2">
                    <span className="font-bold text-[var(--text-main)]">{n.perfis?.nome || "—"}</span>
