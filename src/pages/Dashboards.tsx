...
import { 
  ...
  ComposedChart, Scatter, RadialBarChart, RadialBar, ScatterChart, ZAxis
} from "recharts";
...
export const Dashboards = () => {
  ...
  const loadStats = async () => {
    ...
    try {
      ...
      const res = await api.get(`/dashboard/stats?${q}`);
      setData(res.data);
    } catch {
  ...
  const formatChartData = (rawResult: any[], keyName: string) => {
    if (!rawResult) return [];
    return rawResult.map(item => ({
      name: item[keyName] || "N/A",
      value: item._count ? item._count.id : item.value || 0
    })).sort((a,b) => b.value - a.value);
  };

  const ufData = formatChartData(data.charts.uf, 'uf_atuacao');
  const tituloData = formatChartData(data.charts.titulo, 'titulo');
  const atuacaoData = formatChartData(data.charts.atuacao, 'atuacao_profissional').slice(0, 8);
  const associacaoData = formatChartData(data.charts.associacao, 'name');
  const temporal = data.charts.temporal || [];

  const prodTipoData = formatChartData(data.charts.producao_tipo, 'tipo_obra');
  const prodAreaData = formatChartData(data.charts.producao_area, 'area_processo');

  const { total, ibdp, abep, docentes, doutoras, comLattes, totalProducao, rankingTotal, rankingVotos, totalInstituicoes } = data.kpis;

  const colors = ["#863F56", "#C18C3F", "#4A6741", "#5C5C7A", "#7A7A76"];
  
  const mainKPIs = [
    { title: "Total Mapeado", value: total, sub: "Processualistas" },
    { title: "Membros IBDP", value: ibdp, sub: `${total ? Math.round((ibdp/total)*100) : 0}% da base` },
    { title: "Membros ABEP", value: abep, sub: `${total ? Math.round((abep/total)*100) : 0}% da base` },
    { title: "Docentes Ativas", value: docentes, sub: "Lecionam em IES" },
    { title: "Ranking 40+", value: rankingTotal, sub: `${totalInstituicoes} Instituições Totais` },
    { title: "Votos Ranking", value: rankingVotos, sub: "IES no Ranking 40+" },
    { title: "Doutoras", value: doutoras, sub: "Título de Doutorado" },
    { title: "Obras Mapeadas", value: totalProducao || 0, sub: "Desde 2015" },
  ];
  ...
  return (
    <div className="space-y-8 pb-10 fade-in max-w-7xl mx-auto px-4">
      ...
      {/* Primary Charts - Bento Grid Layout */}
      <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
        
        {/* Grande: Distribuição por UF */}
        ...
        <div className="md:col-span-4 bg-white p-6 rounded-2xl border border-[var(--border)] shadow-sm flex flex-col">
          ...
          <div className="flex-1 min-h-[300px]">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={associacaoData} cx="50%" cy="50%" innerRadius={60} outerRadius={90} paddingAngle={5} dataKey="value">
                  {associacaoData.map((_, index) => <Cell key={`cell-${index}`} fill={colors[index % colors.length]} />)}
                </Pie>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
           <div className="bg-white p-6 rounded-2xl border border-[var(--border)] shadow-sm">
             ...
             <div className="h-[300px]">
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie data={prodTipoData} cx="50%" cy="50%" outerRadius={100} dataKey="value" label={({name, percent}) => `${name} ${(percent * 100).toFixed(0)}%`}>
                      {prodTipoData.map((_, index) => <Cell key={`cell-${index}`} fill={colors[index % colors.length]} />)}
                    </Pie>
                    <Tooltip />
                  </PieChart>
                </ResponsiveContainer>
