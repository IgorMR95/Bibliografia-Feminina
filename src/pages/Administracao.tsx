...
import { ShieldAlert, Trash2, UserPlus, KeyRound, Clock } from "lucide-react";
...
export const Administracao = () => {
  ...
  // Create / Edit modal state for users
  ...
  const loadData = async () => {
    try {
      const [resUsers, resAudit, resTabelas, resCampos] = await Promise.all([
        api.get("/usuarios"),
        api.get("/audit"),
        api.get("/dynamic/tabelas"),
        api.get("/dynamic/campos-extras")
      ]);
      setUsers(resUsers.data);
      setAuditLogs(resAudit.data);
      setTabelas(resTabelas.data);
      setCamposExtras(resCampos.data);
    } catch {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (isEditing) {
        await api.put(`/usuarios/${isEditing.id}`, formData);
      } else {
      ...
      alert("Erro ao salvar usuário. Email pode já estar em uso.");
    }
  };
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
       ...
       <div className="flex border-b border-[var(--border)] overflow-x-auto whitespace-nowrap">
         <button 
           className={`px-4 py-3 font-semibold text-sm ${activeTab === 'usuarios' ? 'border-b-2 border-[var(--accent)] text-[var(--accent)]' : 'text-[var(--text-muted)] hover:text-[var(--text-main)]'}`}
           onClick={() => setActiveTab('usuarios')}
         >
           Usuários
         </button>
         <button 
           className={`px-4 py-3 font-semibold text-sm ${activeTab === 'auditoria' ? 'border-b-2 border-[var(--accent)] text-[var(--accent)]' : 'text-[var(--text-muted)] hover:text-[var(--text-main)]'}`}
           onClick={() => setActiveTab('auditoria')}
         >
           Histórico de Alimentação
         </button>
         <button 
           className={`px-4 py-3 font-semibold text-sm ${activeTab === 'campos' ? 'border-b-2 border-[var(--accent)] text-[var(--accent)]' : 'text-[var(--text-muted)] hover:text-[var(--text-main)]'}`}
           onClick={() => setActiveTab('campos')}
         >
           Campos Base Extras
         </button>
         <button 
           className={`px-4 py-3 font-semibold text-sm ${activeTab === 'tabelas' ? 'border-b-2 border-[var(--accent)] text-[var(--accent)]' : 'text-[var(--text-muted)] hover:text-[var(--text-main)]'}`}
             ...
             <div className="space-y-4">
               {auditLogs.map(log => (
                 <div key={log.id} className="flex items-start bg-[var(--row-hover)] p-4 rounded-lg border border-[var(--border)]">
                   <div className="mr-4 mt-1 text-[var(--text-muted)]">
                     <Clock className="w-5 h-5" />
                   </div>
                   <div className="flex-1">
                     <p className="text-sm font-semibold text-[var(--text-main)]">
                       {log.usuario.nome} <span className="font-normal text-[var(--text-muted)]">realizou ação de</span> <span className="uppercase text-[var(--accent)] font-bold text-xs">{log.acao}</span>
                     </p>
                     <p className="text-xs text-[var(--text-muted)] mt-1">
                       Entidade: {log.entidade} (ID: {log.entidadeId || 'N/A'})
                     </p>
                     {log.detalhes && (
            ...
            <div className="flex justify-between items-center mb-4">
              ...
              <button 
                onClick={async () => {
                  ...
                  if(!label) return;
                  await api.post("/dynamic/campos-extras", { entidade: "Associada", nome_campo: nome, tipo: "string", label });
                  loadData();
                }}
                 ...
                 {camposExtras.map(c => (
                   <tr key={c.id}>
                     ...
                     <td className="px-4 py-3 text-right">
                       <button onClick={async () => { if(confirm("Deletar campo?")){ await api.delete(`/dynamic/campos-extras/${c.id}`); loadData(); } }} className="text-red-500">Remover</button>
                     </td>
                   </tr>
                 ))}
              </tbody>
            ...
            <div className="flex justify-between items-center mb-4">
              ...
              <button 
                onClick={async () => {
                  ...
                  if(!campos) return;
                  await api.post("/dynamic/tabelas", { nome, campos });
                  loadData();
                }}
                className="px-4 py-2 bg-[var(--accent)] text-white text-sm font-semibold rounded-lg hover:bg-[var(--accent-hover)] transition shadow-sm"
              >
              ...
              <tbody className="divide-y divide-[var(--border)]">
                 ...
                 {tabelas.map(t => (
                   <tr key={t.id}>
                     ...
                     <td className="px-4 py-3 text-slate-500 max-w-sm truncate">{t.campos}</td>
                     <td className="px-4 py-3 text-right">
                       <button onClick={async () => { if(confirm("CUIDADO: Remover a tabela apagará todos os dados anexados a ela. Continuar?")){ await api.delete(`/dynamic/tabelas/${t.id}`); loadData(); } }} className="text-red-500">Remover Tabela</button>
                     </td>
                   </tr>
                 ))}
              </tbody>
